...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  _load() async {
    ...
    // copy. Otherwise live learners could see a plausible local rubric
    // instead of the one they are actually scored against — the same class
    ...
    if (path.nodes.isNotEmpty) {
      try {
        rubric =
            (await widget.repository.getMission(
              path.nodes.first.missionId,
            )).rubric;
      } catch (_) {
    ...
    return (progress: progress, receipts: receipts, path: path, rubric: rubric);
  }
