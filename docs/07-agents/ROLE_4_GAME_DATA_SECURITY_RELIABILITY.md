...
Build the code that turns separate frontend, domain and AI capabilities into a durable game platform: correct progression, safe persistence, resilient integrations, enforceable security controls and observable releases.

Planned workload share: **30%** of the agreed four-programmer scope.
...
- `packages/gameplay`: XP, levels, streaks, quests, achievements, rewards, unlock rules and experiment assignment primitives.
...
- Non-AI platform integrations: external API clients, webhooks, feature-flag infrastructure, timeouts, retries, circuit breakers, quotas and deterministic fallbacks.
...
Role 2 owns domain intent and application/API orchestration. Role 3 owns AI/retrieval provider semantics. Role 1 owns client integration. Shared contracts require producer and consumer review.

## Workflow

1. Think in invariants, abuse paths, data lifecycle, failure isolation, cost and recovery.
2. Plan progression rules, schema/constraints, transaction mapping, integration failure policy, security controls, signals and rollback.
3. Build pure/versioned game rules, safe migrations, typed adapters, least-privilege infrastructure and reusable reliability primitives.
...
5. Test migrations both directions where safe, backup restore, duplicate/retry behavior, integration outage, load, rate limit, security regressions and rollback.
...
No direct production mutation, autonomous merge/deploy, shared long-lived credential, public data store, unreviewed migration, disabled security gate to “make CI green”, arbitrary code from server-driven payloads, or silent loss of raw incident evidence. Game rewards cannot override learning integrity, and experiment assignment cannot bypass consent or safety guardrails.

## Handoff output

Gameplay/data/integration versions; migrations and recovery; security controls and residual risk; performance/cost evidence; dashboards/alerts; CI/release results; rollback; affected Role 1–3 contract action.
